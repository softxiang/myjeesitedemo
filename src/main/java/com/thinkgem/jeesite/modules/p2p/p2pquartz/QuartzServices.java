...
import com.thinkgem.jeesite.modules.p2p.p2pquartz.parser.ProxyJob;
...
public class QuartzServices {
	...
	public boolean addJob(final Cp2pParserconfig cp2pParserconfig) {
		...
		if (!CronExpression.isValidExpression(cp2pParserconfig.getCronex())) {
			log.error("任务 [" + cp2pParserconfig.getClassname() + "]---------------未添加成功，时间表达式错误！！！");
			return result;
		}
		String jobName = "";
		String jobGroup = "";
		if (true) {
			jobName = cp2pParserconfig.getClassname().substring(cp2pParserconfig.getClassname().lastIndexOf("."));
		}
		if (true) {
			jobGroup = jobName;
		}
		try {
			...
			JobDetail jobDetail = newJob(ProxyJob.class).withIdentity(jobName, jobGroup).build();
			jobDetail.getJobDataMap().put("cp2pParserconfig", cp2pParserconfig);
			Cp2pSeries cp2pSeries = cp2pSeriesService.get(cp2pParserconfig.getCp2pSeries());
			jobDetail.getJobDataMap().put("cp2pParserconfig", cp2pParserconfig);
			jobDetail.getJobDataMap().put("cp2pSeries", cp2pSeries);
			
			CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(cp2pParserconfig.getCronex());
			trigger = TriggerBuilder.newTrigger().withIdentity(jobName, jobGroup).withSchedule(scheduleBuilder).build();

			quartzScheduler.scheduleJob(jobDetail, trigger);
			result = true;
			log.error("任务 [" + cp2pParserconfig.getClassname() + "]---------------添加成功");
		} catch (SchedulerException e) {
			log.error("任务 [" + cp2pParserconfig.getClassname() + "]---------------未添加成功，消息:" + e.getLocalizedMessage());
			e.printStackTrace();
		}
		return result;
	}
	...
	public boolean deleteJob(Cp2pParserconfig cp2pParserconfig) {
		...
		if (null != triggerKey) {
			try {
				...
				log.info("任务 [" + cp2pParserconfig.getClassname() + "]---------------删除成功！");
			} catch (SchedulerException e) {
				log.error("任务 [" + cp2pParserconfig.getClassname() + "]---------------删除失败！消息：" + e.getLocalizedMessage());
				e.printStackTrace();
			}
		}
		return result;
